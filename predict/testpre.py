...
def run_LR(input_path: str, output_path:str) -> str:
    LR_train = pd.read_csv('data/simpletrainset.csv', sep=';',nrows=10000)
    LR_test = pd.read_csv(f"{output_path}/simpletestset.csv", sep=';',nrows=10000)
    # begin training process
    #print(LR_train.columns)
    X = LR_train.drop(columns='sales')
    #print(LR_train.columns)
    y = LR_train.sales
...
def run_RF(input_path: str, output_path:str) -> str:
    RF_train = pd.read_csv(f"{input_path}/simpletrainset.csv", sep=';',nrows=10000,delim_whitespace=True)
    RF_test = pd.read_csv(f"{input_path}/simpletestset.csv", sep=';',nrows=10000,delim_whitespace=True)
    # begin training process
    X = RF_train.drop(columns='sales')
    y = RF_train['sales']
...
def run_xgboost(input_path:str,output_path:str)->str:
    ...
    train_set = pd.read_csv(f"{input_path}/trainset.csv", sep=';',nrows=10000)
    #print(train_set.columns)
    X_train = train_set.drop(columns=['date', 'sales', 'year'], axis=1)
    y = np.log(train_set['sales'] + 1)

    test_set = pd.read_csv(f"{input_path}/testset.csv", sep=';',nrows=5000)
    print(test_set.columns)
    X_test = test_set.drop(columns=['date', 'year'], axis=1)
    #start = time.time()
    ...
    X_tr = X_train.loc[trn_idx, :].drop(columns=['is_train', 'is_valid'], axis=1)
    X_val = X_train.loc[val_idx, :].drop(columns=['is_train', 'is_valid'], axis=1)
    y_tr = y[trn_idx]
    y_val = y[val_idx]
    ...
    sub_process.to_csv(f'{output_path}/XGB_Model_final1.csv', index=False, sep=';')
...
if __name__ == "__main__":
    ...
    #run_LR(input_path, output_path)
    run_xgboost(input_path, output_path)
