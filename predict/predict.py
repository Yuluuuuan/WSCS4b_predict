...
def run_LR(input_path: str, output_path:str) -> str:
    LR_train = pd.read_csv(f"{input_path}/simpletrainset.csv", sep=';',nrows=10000)
    LR_test = pd.read_csv(f"{output_path}/simpletestset.csv", sep=';',nrows=5000)
...
def run_RF(input_path: str, output_path:str) -> str:
    RF_train = pd.read_csv(f"{input_path}/simpletrainset.csv", sep=';',nrows=10000)
    RF_test = pd.read_csv(f"{input_path}/simpletestset.csv", sep=';',nrows=5000)
    # begin training process
    X = RF_train.drop(columns='sales', axis=1)
    y = RF_train['sales']
...
def run_xgboost(input_path:str,output_path:str)->str:
    ...
    X_train = train_set.drop(columns=['date', 'sales', 'year'])
